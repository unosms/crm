            ...
            <div class="rounded-lg bg-white p-6 shadow-sm ring-1 ring-gray-200">
                <form method="POST" action="{{ route('tickets.store') }}" class="space-y-4">
                        ...
                        <div>
                            ...
                            <select name="client_id" required class="w-full rounded border-gray-300">
                                ...
                                @foreach ($clients as $client)
                                    <option value="{{ $client->id }}" @selected((string) old('client_id', request('client_id')) === (string) $client->id)>{{ $client->full_name }}</option>
                                @endforeach
                            </select>
                        ...
                        <div>
                            ...
                            <select name="status" class="w-full rounded border-gray-300">
                                @foreach ($statuses as $status)
                                    <option value="{{ $status }}" @selected(old('status', 'open') === $status)>{{ str_replace('_', ' ', ucfirst($status)) }}</option>
                                @endforeach
                            </select>
                    ...
                    <div>
                        ...
                        <input name="subject" value="{{ old('subject') }}" required class="w-full rounded border-gray-300">
                        @error('subject') <p class="mt-1 text-sm text-red-600">{{ $message }}</p> @enderror
                    </div>

                    <div>
                        <label class="mb-1 block text-sm font-medium text-gray-700">Description</label>
                        <textarea name="description" rows="6" required class="w-full rounded border-gray-300">{{ old('description') }}</textarea>
                        @error('description') <p class="mt-1 text-sm text-red-600">{{ $message }}</p> @enderror
                    </div>

                    <div class="flex gap-3">
                        <button class="rounded bg-indigo-600 px-4 py-2 text-sm font-semibold text-white hover:bg-indigo-700">Create Ticket</button>
                        <a href="{{ route('tickets.index') }}" class="rounded border border-gray-300 px-4 py-2 text-sm font-semibold text-gray-700 hover:bg-gray-50">Cancel</a>
                    </div>
                </form>
            </div>
        </div>
